...
const setUser = (state: IUserState, { user }: { user: IUser }) => ({ ...state, user });
const removeUser = (state: IUserState) => ({ ...state, user: null });

const _userReducer = createReducer<IUserState>(initialUserState, on(UserLoaded, setUser), on(UserLoggedOut, removeUser));
